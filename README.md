...
Frame is being developed as a voice-first personal workspace: capture a thought or conversation, review what matters, recover original context and prepare a useful next step. Frame 3.0.0 (4) compiled with 81 passing automated checks and one device-only skip. Apple processed it and assigned it to Owner Testing (Internal, one tester), with What to Test saved. New-build installation and physical-device acceptance remain unverified. Optional AI uses Apple’s on-device model on compatible devices with iOS 26.4 or later. English output and supported on-device transcription only; mixed-language accuracy is unverified. Frame does not send messages automatically or record other-app calls. Earlier video-prototype work is retained separately.
