...
| --- | --- | --- |
| [Quote Operations](https://kbartawi.github.io/projects/quote-operations/) | Equipment requests into quotations a team can review and approve | Working demo · Pilot preparation |
...
Frame is being developed as a voice-first personal workspace: capture a thought or conversation, review what matters, recover original context and prepare a useful next step. Build 3.0.0 (4) remains verified in Owner Testing (Internal, one tester). Signed build 3.0.0 (6) uploaded at 11:35:59 Dubai on 20 September; Apple processing and assignment remain unverified. The latest candidate includes local reminders, clearer recording/save guidance and a control to reveal all actions. It passed 25 targeted checks with one device-only skip and 27 iOS 18.5 compatibility checks; Simulator UI verified finding and completing a fourth action. Physical-device installation and acceptance remain unverified. Optional AI uses Apple’s on-device model on compatible devices with iOS 26.4 or later. English output and supported on-device transcription only; mixed-language accuracy is unverified. Frame does not send messages automatically or record other-app calls. Earlier video-prototype work is retained separately.
